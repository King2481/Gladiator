...
UCLASS()
class GLADIATOR_API APickup : public AActor
{
	GENERATED_BODY()
...
protected:
	...
	virtual bool CanPickup(AGladiatorCharacter* ForUser) const;

	// Blueprint event for CanPickup()
	UFUNCTION(BlueprintImplementableEvent, Category = "Pickup")
	bool BlueprintCanPickup(AGladiatorCharacter* ForUser) const;

	// Handles the picking up of the item
	virtual void HandlePickup(AGladiatorCharacter* ForUser);

	// Blueprint event for picking up the item.
	UFUNCTION(BlueprintImplementableEvent, Category = "Pickup")
	void BlueprintHandlePickup(AGladiatorCharacter* ForUser);
